    ...
    TimerTask timerTask = new TimerTask() {
        ...
        public void run() {
            ...
            for(int i = 0; i < game.countApples; i ++){
                game.apples[i].secOnBoard ++;
                if(game.apples[i].secOnBoard >= 10){
                    game.apples[i].fresh = false;
                    game.apples[i].color = blue;
                    game.apples[i].rewardPoints = -20;
                    game.apples[i].lengthenTail = -8;
                }
            }
...
TApple createApple(TPoint coordinates, int color){
    TApple apple = new TApple();
    setApple(apple, coordinates, color);
    return apple;
}

void setApple(TApple apple, TPoint coordinates, int color){
    apple.coordinates = coordinates;
    apple.color = color;
    apple.fresh = true;
    switch(color){
        case green:
            apple.rewardPoints = 10;
            apple.lengthenTail = 4;
            break;
        case red:
            apple.rewardPoints = 20;
            apple.lengthenTail = 8;
            break;
    }
    apple.secOnBoard = 0;
    apple.eaten = false;
...
void shiftSnake(TGame game, TPlayer player){
    if(player.body.pointsCount < 2) return;
    for(int i = 1; i <= player.body.pointsCount; i ++){
        player.body.points[i - 1] = player.body.points[i];
    }

}

void addApple(TGame game){
    int[] colors = {green, red};
    int lastIndex = game.countApples - 1;
    int appleX = (int) (Math.random()*(game.board.x2 - game.board.x1 - 2)) + game.board.x1 + 1;
    int appleY = (int) (Math.random()*(game.board.y2 - game.board.y1 - 2)) + game.board.y1 + 1;
    while(isPointInPath(point(appleX, appleY), game.player1.body) || isPointInPath(point(appleX, appleY), game.player2.body)){
        appleX = (int) (Math.random()*(game.board.x2 - game.board.x1 - 2)) + game.board.x1 + 1;
        appleY = (int) (Math.random()*(game.board.y2 - game.board.y1 - 2)) + game.board.y1 + 1;
    }
    int color = (int) (Math.random()*2);
    game.apples[lastIndex + 1] = createApple(point(appleX, appleY), colors[color]);
    game.countApples ++;
}

void updateTime(TGame game){
    int minutesInt = game.time / 60;
    int secondsInt = game.time % 60;
    String minutesStr;
    String secondsStr;
    if(minutesInt < 10) minutesStr = "0"+minutesInt;
    else minutesStr = "" +  minutesInt;
    if(secondsInt < 10) secondsStr = "0"+secondsInt;
    else secondsStr = "" + secondsInt;
    game.board.frame.printDirection = 0;
    setfgcolor(game.board.color);
    writeStringOnPath(minutesStr + ":" + secondsStr, game.board.frame, 3);
}


void initBoard(TGame game){
    clrscr();
    setGame(game, createBoard(1,120,1,30, green), createPlayer(point(3,15), "Player1", blue, 1, 1, 0), createPlayer(point(117,15), "Player2", red, 1, -1, 0));
    setfgcolor(game.board.color);
    int middleTop = (game.board.x1 + game.board.x2) / 2;
    int middleBottom = 3 * middleTop + (game.board.y2 - game.board.x1);
    String upperString = "SNAKE GAME";
...
void eatApple(TGame game, TPlayer player){
    TPoint playerHead = player.body.points[player.body.pointsCount - 1];
    for(int i = 0; i < game.countApples; i++){
        if(pointsEqual(game.apples[i].coordinates, playerHead) && !game.apples[i].eaten){
            player.points += game.apples[i].rewardPoints;
            player.pointsToAdd = game.apples[i].lengthenTail;
            game.apples[i].eaten = true;
            break;
        }
    }
}
...
void lengthenOrShortenSnake(TGame game, TPlayer player){
    if(player.pointsToAdd == 0){
        shiftSnake(game, player);
        player.body.pointsCount--;
    }
    else if(player.pointsToAdd < 0){
        if(player.body.pointsCount + player.pointsToAdd <= 0) player.hitFlag = true;
        else{
            shiftSnake(game,player);
            player.body.pointsCount --;
            writeStringOnPath(" ", player.body, 0);
            shiftSnake(game,player);
            player.body.pointsCount --;
            player.pointsToAdd++;
            writeStringOnPath(" ", player.body, 0);
    ...
    else player.pointsToAdd--;
}

void drawNewStep(TGame game, TPlayer player){
    int lastIndex = player.body.pointsCount - 1;
    TPoint lastPoint = player.body.points[lastIndex];
    ...
    lengthenOrShortenSnake(game,player);
    setfgcolor(player.color);
    writeStringOnPath("*#", player.body, lastIndex - 1);

}

void printPoints(TGame game){
    String points1Str;
    String points2Str;
    if(game.player1.points < 10) points1Str = "00"+game.player1.points;
    else if (game.player1.points<100) points1Str = "0" + game.player1.points;
    else points1Str = "" + game.player1.points;

    if(game.player2.points < 10) points2Str = "00"+game.player2.points;
    else if (game.player2.points<100) points2Str = "0" + game.player2.points;
    else points2Str = "" + game.player2.points;

    game.board.frame.printDirection = 0;
    setfgcolor(game.player1.color);
    writeStringOnPath(game.player1.name + ": " + points1Str, game.board.frame, game.board.x2 - 30);
    setfgcolor(game.player2.color);
    writeStringOnPath(game.player2.name + ": " + points2Str, game.board.frame, game.board.x2 - 17);

}

void drawApples(TGame game){
    int lastIdx = game.countApples;
    for(int i = 0; i < lastIdx; i++){
        setfgcolor(game.apples[i].color);
        if(!game.apples[i].eaten) drawPoint(game.apples[i].coordinates, '$');
    }
}
