...
void planned_squares2(){
    ...
    while(true){
        ...
        setfgcolor(red);
        draw_frame_c(first_x[i % j1], first_y[i % j1], first_x[i % j1] + 2, first_y[i % j1] + 2, '*');
        setfgcolor(blue);
        draw_frame_c(second_x[i % j2], second_y[i % j2], second_x[i % j2] + 2, second_y[i % j2] + 2, '*');
        setfgcolor(green);
        draw_frame_c(third_x[i % j3], third_y[i % j3], third_x[i % j3] + 2, third_y[i % j3] + 2, '*');
        delay(200);
        draw_frame_c(first_x[i % j1], first_y[i % j1], first_x[i % j1] + 2, first_y[i % j1] + 2, ' ');
        draw_frame_c(second_x[i % j2], second_y[i % j2], second_x[i % j2] + 2, second_y[i % j2] + 2, ' ');
        draw_frame_c(third_x[i % j3], third_y[i % j3], third_x[i % j3] + 2, third_y[i % j3] + 2, ' ');
