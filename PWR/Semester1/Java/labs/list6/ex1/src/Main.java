...
public class Main {
    ...
    public static void addCorner(TCornerPath cornersPath, TPoint newCorner) {
        ...
        else if (cornersPath.cornersCount >= 2) cornersPath.realPointsCount += countSegmentLength(prevLastCorner, newCorner) - 1;
    }
    ...
    public static int countSegmentLength(TPoint point1, TPoint point2) {
        if(arePointsEqual(point1, point2)) return 1;
        return Math.max(Math.abs(point1.x - point2.x), Math.abs(point1.y - point2.y)) + 1;
    }
