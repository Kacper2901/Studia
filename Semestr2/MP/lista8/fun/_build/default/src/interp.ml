...
let rec eval_env (env : env) (e : expr) : value =
  ...
  | IsPair e ->
      (match eval_env env e with
        | VPair _ -> VBool true
        | _ -> VBool false)
  | Fun (x, e) -> VClosure (x, e, env) (*pamieta zmienna cialo i srodoeidko funkji*)
  | Funrec (f, x, e) -> VRecClosure (f, x, e, env) (*dodatkowo pamieta funkcje zeby moc z niej korzystac*)
  | App (e1, e2) ->
      let v1 = eval_env env e1 in (*e1 evaluuje sie do rodzaju funckji*)
      let v2 = eval_env env e2 in (*e2 evaluuje sie do wartosci argumentow (bo mogly byc w tym srodowisku)*)
      (match v1 with
        | VClosure (x, body, clo_env) ->
            eval_env (M.add x v2 clo_env) body (*wykonujemy cialo funkci tworzac nowe srodowisko z argumentami*)
        | VRecClosure (f, x, body, clo_env) as c ->
            eval_env (clo_env |> M.add x v2 |> M.add f c) body (*to samo co wczesniej ale jeszcze dodajemy funkcje do srodowiska*)
