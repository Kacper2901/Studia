...
let rec eval (e : expr) : value =
  ...
  | Int i -> VInt i
  | Bool b -> VBool b
  | Binop (And, e1, e2) -> (match (eval e1) with
                           | VBool false -> VBool false
                           | VBool true -> eval e2
                           | _ -> failwith "type error")
  | Binop (Or, e1, e2) -> (match (eval e1) with
  ...
  | NumberQ e -> (match eval e with
                 | VInt _-> VBool true
                 | _ -> VBool false)
  | BooleanQ e ->( match eval e with
                 | VBool _ -> VBool true
                 | _ -> VBool false)
  | PairQ e -> (match eval e with
                 | VPair _ -> VBool true
                 | _ -> VBool false)
  | UnitQ e -> (match eval e with
                 | VUnit -> VBool true
                 | _ -> VBool false)
