...
module I = Interp (*alias modulu interp*)
...
let rec find_index (x : Ast.ident) (env : env) : int = (*liczy ile "krokow" trzeba zrobic zeby dojsc do zmiennej w env*)
  match env with
  | y :: _ when x = y -> 0
...
(*zad1*)
let rec to_PN (env : env) (expr : Ast.expr) : prog =
    match expr with
    | Ast.Int n ->
        [PushInt n]
    | Ast.Bool b ->
        [PushBool b]
    | Ast.Binop (op, e1, e2) ->
        [Binop op] @ to_PN env e1 @ to_PN env e2
    | Ast.If (b, t, e) ->
        [CndJmp (to_PN env t, to_PN env e)] @ to_PN env b
    | Ast.Pair (e1, e2) ->
        [PushPair] @ to_PN env e1 @ to_PN env e2
    | Ast.Fst e ->
        [Fst] @ to_PN env e
    | Ast.Snd e ->
        [Snd] @ to_PN env e
    ...
    | Ast.Var x ->
        [Load (find_index x env)]
    | Ast.Let (x, e1, e2) ->
        [Store] @ to_PN env e1 @ to_PN (x :: env) e2 @ [CleanUp]
    | _ ->
        failwith "not implemented"
...
let stack_size (prog : cmd list) : int =
    let rec aux cmds current_depth max_depth =
      match cmds with
      | [] -> max_depth
      | cmd :: rest ->
        let temp =
          match cmd with
          | PushInt _ | PushBool _ | PushUnit | Load _|PushVar _ -> 1 (*dodaje pojedyncze elementy*)
          | PushPair | Binop _ -> -1 (*zdejmuje ze stosu dwa elementy i daje jeden*)
          | Fst | Snd | IsPair -> 0 (*bierze element i zwraca element (np bool w IsPair)*)
          | CndJmp (_, _) -> -2 (*zdejmuje 3 elementy: c,t,e i daje jeden (then / else)*)
          | Store | CleanUp -> -1 (*sciaga ze stosu i zapisuje w srodowisku / usuwa*)
        in
        let new_depth = current_depth + temp in
        let new_max = max max_depth new_depth in
        aux rest new_depth new_max
    in
    aux prog 0 0
 ...
 *)
 ...
 (* Środowisko zmieniamy na listę par (identyfikator, wartość) *)
...
let rec eval (env : I.value list) (s : I.value list) (p : prog) : I.value =
  ...
  (*zad4*)
  let rec eval_env (env : (string * I.value) list) (s : I.value list) (p : prog) : I.value =
    match p, s with
    | [], [n] -> n
    | [], _   -> failwith "error!"
  
    | (PushInt n :: p'), _ ->
        eval_env env (I.VInt n :: s) p'
  
    | (PushBool b :: p'), _ ->
        eval_env env (I.VBool b :: s) p'
  
    | (PushVar x :: p'), _ ->  (* zadanie 4 *)
        let v = lookup x env in
        eval_env env (v :: s) p'
  
    | (Binop op :: p'), (v2 :: v1 :: s') ->
        eval_env env (I.eval_op op v1 v2 :: s') p'
    ...
    | (PushPair :: p'), (v2 :: v1 :: s') ->
        eval_env env (I.VPair (v1, v2) :: s') p'
    ...
    | (PushUnit :: p'), s' ->
        eval_env env (I.VUnit :: s') p'
  
    | (IsPair :: p'), (I.VPair _ :: s') ->
        eval_env env (I.VBool true :: s') p'
  
    | (IsPair :: p'), (_ :: s') ->
        eval_env env (I.VBool false :: s') p'
  
    | (Store :: p'), (v :: s') ->
        eval_env (("_tmp", v) :: env) s' p'
  
    | (Load n :: p'), _ ->
        let env_vals = List.map snd env in
        eval_env env (List.nth env_vals n :: s) p'
  
    | (CleanUp :: p'), _ ->
        eval_env (List.tl env) s p'
